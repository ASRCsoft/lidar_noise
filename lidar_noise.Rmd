...
```{python, fig.cap='RWS data and results', fig.align='center', fig.width=3.17}
...
# organize for R
rws = lidar2_0['RWS'].values
times = lidar2_0.coords['Time'].values
ranges = lidar2_0.coords['Range'].values
```

```{r filters, fig.cap='Raw and filtered CNR and RWS values for October 3rd, 2017', fig.width=3.17}
library(reshape2)
library(ggplot2)
library(scales)
...
# ggplot(rws_df, aes(x=Time, y=Range, fill=value, color=value)) + geom_raster() +
...
```{r corr, fig.cap='Pearson correlation of lidar horizontal wind speeds and radiosonde wind speeds at different heights.', fig.width=3.17}
library(reshape2)
library(ggplot2)
